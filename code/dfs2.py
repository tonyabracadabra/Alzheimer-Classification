...
class DeepFeatureSelectionNew:
    def __init__(self, X_train, X_test, y_train, y_test, weight_init='sda', n_input = 2, hidden_dims=[1000],
                 activation='sigmoid',epochs=1000, dropout=[True], lambda1=0.001, lambda2=1.0, alpha1=0.001,
                 alpha2=0.0, learning_rate=0.1, optimizer='FTRL', print_step=1000, tolerance = 5):
        ...
        assert len(hidden_dims) == len(dropout)
        ...
        for init_w, init_b in zip(weights, biases):
            ...
            if dropout[i] != False:
                input_hidden = tf.nn.dropout(input_hidden, tf.constant(dropout[i]))
            i += 1

        # Final classification layer, variable Y is passed
        self.softmax_layer = SoftmaxLayer(input_hidden, n_classes, self.var_Y)
